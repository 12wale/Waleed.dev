...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center px-4 md:px-8 overflow-hidden bg-surface" id="hero">
      ...
      {/* Interactive Floating Glass Shapes */}
      {/* Shape 1: Square */}
      <div 
        className="absolute top-[20%] left-[10%] w-32 h-32 md:w-48 md:h-48 bg-primary/10 rounded-2xl backdrop-blur-lg border border-primary/20 transition-transform duration-700 ease-out shadow-[0_0_50px_rgba(189,157,255,0.2)] rotate-12"
        style={{ transform: `translate(${mousePosition.x * -2}px, ${mousePosition.y * -2}px) rotate(12deg)` }}
      ></div>
      
      {/* Shape 2: Circle */}
      <div 
        className="absolute bottom-[15%] right-[10%] w-40 h-40 md:w-64 md:h-64 bg-secondary/10 rounded-full backdrop-blur-xl border border-secondary/20 transition-transform duration-1000 ease-out shadow-[0_0_60px_rgba(255,255,255,0.1)]"
        style={{ transform: `translate(${mousePosition.x * 3}px, ${mousePosition.y * 3}px)` }}
      ></div>
      
      {/* Shape 3: Small floating accent */}
      <div 
        className="absolute top-[40%] right-[25%] w-12 h-12 md:w-20 md:h-20 bg-blue-500/20 rounded-lg backdrop-blur-md border border-blue-400/30 transition-transform duration-500 ease-out shadow-[0_0_30px_rgba(59,130,246,0.3)] animate-pulse hidden md:block"
      ...
      <div className="relative z-10 text-center max-w-4xl px-4 mt-16 md:mt-0 w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
          <a className="group relative px-8 py-4 bg-gradient-to-r from-[#bd9dff] to-[#ffffff] rounded-xl text-[#0a1128] font-extrabold title-sm transition-all duration-300 hover:scale-105 hover:shadow-[0_0_40px_rgba(189,157,255,0.6)] overflow-hidden w-full sm:w-auto" href="#projects">
          ...
          <a className="group relative px-8 py-4 border border-outline-variant rounded-xl text-white font-bold title-sm hover:bg-surface-variant transition-all duration-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:-translate-y-1 w-full sm:w-auto bg-[#192540]/30 backdrop-blur-md" href="/Waleed_Refaat_CV_Updated.pdf" download="Waleed_Refaat_CV.pdf">
            Download CV
